...
router.get("/:id", auth, async (req, res) => {
  try {
    ...
    // Check authorization
    if (
      attempt.userId._id.toString() !== req.user.userId.toString() &&
      req.user.role !== "admin"
    ) {
      return res.status(403).json({ message: "Access denied" });
    }
...
router.get("/", auth, adminAuth, async (req, res) => {
  try {
    const { testId, userId, flagged, page = 1, limit = 20 } = req.query;
...
router.get("/stats/overview", auth, adminAuth, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Server error fetching attempt statistics" });
  }
});
